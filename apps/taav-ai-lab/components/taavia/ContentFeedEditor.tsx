...
import { Image as ImageIcon, Loader2, Mic, Paperclip, Send, StopCircle, Trash2, Type, Video } from 'lucide-react';
...
export function ContentFeedEditor({
  ...
  const [activeInput, setActiveInput] = useState<'text' | null>(null);
  ...
  const toggleTextComposer = () => {
    if (disabled) return;
    setActiveInput((current) => (current === 'text' ? null : 'text'));
  };
  ...
  const isTextComposerVisible = activeInput === 'text';

  return (
    <div className="flex min-h-[420px] flex-col rounded-[28px] border border-white/10 bg-[linear-gradient(180deg,rgba(18,30,56,0.78)_0%,rgba(10,19,38,0.78)_100%)] shadow-[0_18px_48px_rgba(0,0,0,0.22)] lg:h-full lg:min-h-0">
      ...
      <div
        className="flex-1 overflow-y-auto px-4 py-5 md:px-6 lg:min-h-0"
        onDragOver={(event) => {
      ...
      >
        {messages.length > 0 ? (
          ...
          <div className="mx-auto grid h-full min-h-[280px] w-full max-w-3xl place-items-center rounded-[22px] border border-dashed border-white/14 bg-white/5 p-8 text-center">
      ...
      <div className="border-t border-white/10 bg-[rgba(5,12,25,0.55)] p-4 md:p-5">
        <div className="mx-auto w-full max-w-4xl rounded-[28px] border border-[rgba(130,158,255,0.14)] bg-[linear-gradient(180deg,rgba(9,18,34,0.96)_0%,rgba(5,11,23,0.98)_100%)] px-4 py-4 shadow-[0_20px_48px_rgba(0,0,0,0.24)] md:px-5">
          <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
            <div className="flex flex-wrap items-center gap-2">
              <button
                type="button"
                onClick={toggleTextComposer}
                disabled={disabled}
                className={`inline-flex items-center gap-2 rounded-full border px-3 py-2 text-[12px] font-bold transition disabled:opacity-50 ${
                  isTextComposerVisible
                    ? 'border-[rgba(250,204,21,0.28)] bg-[rgba(250,204,21,0.12)] text-[rgb(253,224,71)]'
          ...
          <div
            className={`overflow-hidden transition-all duration-300 ease-out ${
              isTextComposerVisible ? 'max-h-64 opacity-100 translate-y-0 mt-3' : 'max-h-0 opacity-0 -translate-y-2 mt-0'
            }`}
          >
            <div className="flex items-end gap-3 rounded-[24px] border border-white/10 bg-[rgba(2,8,19,0.72)] p-3 md:p-4">
              <button
                type="button"
              ...
              <textarea
                ...
                rows={3}
                disabled={disabled}
                className="min-h-[104px] flex-1 resize-none rounded-[18px] border border-[rgba(130,158,255,0.14)] bg-[rgba(10,18,32,0.96)] px-4 py-3 text-right text-[length:var(--taav-text-sm)] leading-7 text-white outline-none transition placeholder:text-[rgba(217,229,255,0.34)] focus:border-[rgba(66,237,211,0.36)] focus:bg-[rgba(12,22,38,1)] disabled:opacity-60"
              />
            </div>
          </div>
        </div>
