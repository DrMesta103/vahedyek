...
export function TaaviaKnowledgeBaseCategoriesDetailsClient({ data }: { data: KnowledgeBaseCategoryDetailsPageData }) {
  ...
  const overviewHref = `/businesses/${data.businessId}/products/taavia/brands/${data.brandId}/knowledge-base/${data.knowledgeBaseId}`;
    ...
    <main dir="rtl" className="mx-auto grid max-w-[1440px] gap-4 pb-8 text-right">
      <header className="rounded-[var(--taav-radius-xl)] border border-[var(--taav-border-subtle)] bg-[var(--taav-surface)] p-3 shadow-[var(--taav-shadow-sm)]">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <aside className="flex max-w-sm items-center gap-2 rounded-lg border border-sky-400/30 bg-sky-500/[0.08] px-3 py-2 text-xs leading-6 text-sky-200">
              <Info className="h-4 w-4 shrink-0" />
              دسته‌بندی‌ها فقط‌خواندنی هستند و امکان ویرایش وجود ندارد.
            </aside>
            <Link href={overviewHref}>
              <TaavButton size="sm" variant="secondary" iconStart={<ChevronLeft className="h-4 w-4" />}>
                بازگشت
              </TaavButton>
            </Link>
          </div>
      ...
      {/* RTL: first column renders on the right (categories), second on the left (content). */}
      <section className="grid gap-4 lg:grid-cols-[minmax(360px,0.9fr)_minmax(0,1.2fr)]">
            ...
            {parents.map((parent) => (
              <div key={parent.id} className={expanded.has(parent.id) && childrenFor(parent.id).length ? 'border-b border-[var(--taav-border-subtle)]/40' : ''}>
                ...
                {expanded.has(parent.id) ? (
                  <div className="border-r border-sky-400/15 mr-5 bg-black/[0.12]">
            ...
            <div className="mt-2 grid max-h-[min(58vh,560px)] gap-3 overflow-y-auto p-0.5 sm:grid-cols-2 xl:grid-cols-3">
              {resourcesCategory.resources.map((resource) => (
                ...
                >
                  ...
                  <Link href={`/businesses/${data.businessId}/products/taavia/brands/${data.brandId}/knowledge-base/${data.knowledgeBaseId}/sources`} className="mt-auto">
