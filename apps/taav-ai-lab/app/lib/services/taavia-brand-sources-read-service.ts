...
async function requireScopedBrand(userId: string, tenantId: string, brandId: string) {
  if (!(await assertTenantAccess(userId, tenantId))) return null;
  return prisma.taaviaBrand.findFirst({ where: { id: brandId, tenantId }, select: { id: true } });
}
