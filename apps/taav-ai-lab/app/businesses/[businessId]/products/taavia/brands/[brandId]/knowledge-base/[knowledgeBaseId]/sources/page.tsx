...
import { getKnowledgeBaseVersionSourcesPageData } from '@/app/lib/services/taavia-knowledge-base-read-service';
import { getCurrentTenant, requireSession } from '@/app/lib/session';
import { AiLabShell } from '@/components/AiLabShell';
import { TaaviaKnowledgeBaseVersionSourcesClient } from '@/components/taavia/knowledge-base/TaaviaKnowledgeBaseVersionSourcesClient';
...
export default async function Page({ params }: { params: Promise<{ businessId: string; brandId: string; knowledgeBaseId: string }> }) {
  ...
  if (!brand) notFound();
  const data = await getKnowledgeBaseVersionSourcesPageData(session.userId, business.id, brand.id, knowledgeBaseId, brand.name);
  if (!data) notFound();

  return (
    <AiLabShell
      pathname={`/businesses/${business.id}/products/taavia/brands/${brand.id}/knowledge-base/${data.knowledgeBaseId}/sources`}
    ...
    >
      <TaaviaKnowledgeBaseVersionSourcesClient data={data} />
    </AiLabShell>
  );
}
