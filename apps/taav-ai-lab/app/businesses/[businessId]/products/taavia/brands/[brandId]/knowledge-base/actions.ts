...
import {
  advanceInitialKnowledgeBuild,
  cancelInitialKnowledgeBuild,
  completeInitialKnowledgeBuild,
  failInitialKnowledgeBuild,
  resetInitialKnowledgeBuild,
  retryInitialKnowledgeBuild,
  startInitialKnowledgeBuild,
  startRebuildKnowledgeBuild,
  startUpdateKnowledgeBuild,
} from "@/app/lib/taavia-initial-kb-build";
...
export async function startKnowledgeBaseRebuildAction(input: { businessId: string; brandId: string; knowledgeBaseId: string }) {
  const session = await scoped(input.businessId, input.brandId);
  const active = await prisma.taaviaKnowledgeBase.findFirst({
    where: { id: input.knowledgeBaseId, tenantId: input.businessId, brandId: input.brandId, isActive: true },
    select: { id: true },
  });
  if (!active) throw new Error("فقط نسخهٔ فعال قابل ریبلد است.");
  ...
  const build = await startRebuildKnowledgeBuild({
    tenantId: input.businessId,
    brandId: input.brandId,
    userId: session.userId,
  });
...
export async function startKnowledgeBaseNewVersionAction(input: { businessId: string; brandId: string; knowledgeBaseId: string }) {
  return startKnowledgeBaseUpdateAction({ ...input, force: true });
}
