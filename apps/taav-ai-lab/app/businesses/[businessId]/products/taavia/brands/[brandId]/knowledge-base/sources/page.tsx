...
export default async function Page({ params }: { params: Promise<{ businessId: string; brandId: string }> }) {
  ...
  redirect(activeKnowledgeBaseId ? `/businesses/${business.id}/products/taavia/brands/${brand.id}/knowledge-base/${activeKnowledgeBaseId}/sources` : `/businesses/${business.id}/products/taavia/brands/${brand.id}/knowledge-base`);
}
