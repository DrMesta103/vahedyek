...
import {
  GENERATE_KNOWLEDGE_BASE_CARDS,
  GENERATE_KNOWLEDGE_BASE_META,
  GENERATE_KNOWLEDGE_BASE_OVERVIEW_NOTE,
  GENERATE_KNOWLEDGE_BASE_OVERVIEW_STEPS,
} from '@/app/lib/taavia-build-version-generate-knowledge-base';
...
export default async function GenerateKnowledgeBasePage({
  params,
}: {
  ...
  const currentHref = `${overviewHref}/${GENERATE_KNOWLEDGE_BASE_META.slug}`;
    ...
    >
      <div className="ai-lab-page-stack">
        <TaavPageHeader
          ...
          eyebrow="محصول تاویا"
          title={GENERATE_KNOWLEDGE_BASE_META.title}
          description={GENERATE_KNOWLEDGE_BASE_META.description}
          status="active"
          meta={
            <div className="flex flex-wrap items-center gap-2">
              {GENERATE_KNOWLEDGE_BASE_META.pills.map((pill) => (
                <span
                  key={pill}
        ...
        <BuildVersionStepDocClient
          cards={GENERATE_KNOWLEDGE_BASE_CARDS}
          overviewSteps={GENERATE_KNOWLEDGE_BASE_OVERVIEW_STEPS}
          overviewNote={GENERATE_KNOWLEDGE_BASE_OVERVIEW_NOTE}
        />
      </div>
    </AiLabShell>
  );
