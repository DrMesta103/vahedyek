...
import {
  RECEIVE_BRAND_SOURCES_CARDS,
  RECEIVE_BRAND_SOURCES_META,
  RECEIVE_BRAND_SOURCES_OVERVIEW_NOTE,
  RECEIVE_BRAND_SOURCES_OVERVIEW_STEPS,
} from '@/app/lib/taavia-build-version-receive-brand-sources';
...
export default async function ReceiveBrandSourcesPage({
  params,
}: {
  ...
  const currentHref = `${overviewHref}/${RECEIVE_BRAND_SOURCES_META.slug}`;
    ...
    >
      <div className="ai-lab-page-stack">
        <TaavPageHeader
          ...
          eyebrow="محصول تاویا"
          title={RECEIVE_BRAND_SOURCES_META.title}
          description={RECEIVE_BRAND_SOURCES_META.description}
          status="active"
          meta={
            <div className="flex flex-wrap items-center gap-2">
              {RECEIVE_BRAND_SOURCES_META.pills.map((pill) => (
                <span
                  key={pill}
        ...
        <BuildVersionStepDocClient
          cards={RECEIVE_BRAND_SOURCES_CARDS}
          overviewSteps={RECEIVE_BRAND_SOURCES_OVERVIEW_STEPS}
          overviewNote={RECEIVE_BRAND_SOURCES_OVERVIEW_NOTE}
        />
      </div>
    </AiLabShell>
  );
