...
import {
  GET_LATEST_BUILD_STATUS_WITH_REST_CARDS,
  GET_LATEST_BUILD_STATUS_WITH_REST_META,
  GET_LATEST_BUILD_STATUS_WITH_REST_OVERVIEW_NOTE,
  GET_LATEST_BUILD_STATUS_WITH_REST_OVERVIEW_STEPS,
} from '@/app/lib/taavia-build-version-get-latest-build-status-with-rest';
...
export default async function GetLatestBuildStatusWithRestPage({
  params,
}: {
  ...
  const currentHref = `${overviewHref}/${GET_LATEST_BUILD_STATUS_WITH_REST_META.slug}`;
    ...
    >
      <div className="ai-lab-page-stack">
        <TaavPageHeader
          ...
          eyebrow="محصول تاویا"
          title={GET_LATEST_BUILD_STATUS_WITH_REST_META.title}
          description={GET_LATEST_BUILD_STATUS_WITH_REST_META.description}
          status="active"
          meta={
            <div className="flex flex-wrap items-center gap-2">
              {GET_LATEST_BUILD_STATUS_WITH_REST_META.pills.map((pill) => (
                <span
                  key={pill}
        ...
        <BuildVersionStepDocClient
          cards={GET_LATEST_BUILD_STATUS_WITH_REST_CARDS}
          overviewSteps={GET_LATEST_BUILD_STATUS_WITH_REST_OVERVIEW_STEPS}
          overviewNote={GET_LATEST_BUILD_STATUS_WITH_REST_OVERVIEW_NOTE}
        />
      </div>
    </AiLabShell>
  );
