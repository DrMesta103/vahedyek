...
import {
  GET_RESULT_FROM_PYTHON_GRPC_CARDS,
  GET_RESULT_FROM_PYTHON_GRPC_META,
  GET_RESULT_FROM_PYTHON_GRPC_OVERVIEW_NOTE,
  GET_RESULT_FROM_PYTHON_GRPC_OVERVIEW_STEPS,
} from '@/app/lib/taavia-build-version-get-result-from-python-grpc';
...
export default async function GetResultFromPythonGrpcPage({
  params,
}: {
  ...
  const currentHref = `${overviewHref}/${GET_RESULT_FROM_PYTHON_GRPC_META.slug}`;
    ...
    >
      <div className="ai-lab-page-stack">
        <TaavPageHeader
          ...
          eyebrow="محصول تاویا"
          title={GET_RESULT_FROM_PYTHON_GRPC_META.title}
          description={GET_RESULT_FROM_PYTHON_GRPC_META.description}
          status="active"
          meta={
            <div className="flex flex-wrap items-center gap-2">
              {GET_RESULT_FROM_PYTHON_GRPC_META.pills.map((pill) => (
                <span
                  key={pill}
        ...
        <BuildVersionStepDocClient
          cards={GET_RESULT_FROM_PYTHON_GRPC_CARDS}
          overviewSteps={GET_RESULT_FROM_PYTHON_GRPC_OVERVIEW_STEPS}
          overviewNote={GET_RESULT_FROM_PYTHON_GRPC_OVERVIEW_NOTE}
        />
      </div>
    </AiLabShell>
  );
