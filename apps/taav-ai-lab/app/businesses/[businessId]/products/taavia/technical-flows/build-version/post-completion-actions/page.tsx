...
import {
  POST_COMPLETION_ACTIONS_CARDS,
  POST_COMPLETION_ACTIONS_META,
  POST_COMPLETION_ACTIONS_OVERVIEW_NOTE,
  POST_COMPLETION_ACTIONS_OVERVIEW_STEPS,
} from '@/app/lib/taavia-build-version-post-completion-actions';
...
export default async function PostCompletionActionsPage({
  params,
}: {
  ...
  const currentHref = `${overviewHref}/${POST_COMPLETION_ACTIONS_META.slug}`;
    ...
    >
      <div className="ai-lab-page-stack">
        <TaavPageHeader
          ...
          eyebrow="محصول تاویا"
          title={POST_COMPLETION_ACTIONS_META.title}
          description={POST_COMPLETION_ACTIONS_META.description}
          status="active"
          meta={
            <div className="flex flex-wrap items-center gap-2">
              {POST_COMPLETION_ACTIONS_META.pills.map((pill) => (
                <span
                  key={pill}
        ...
        <BuildVersionStepDocClient
          cards={POST_COMPLETION_ACTIONS_CARDS}
          overviewSteps={POST_COMPLETION_ACTIONS_OVERVIEW_STEPS}
          overviewNote={POST_COMPLETION_ACTIONS_OVERVIEW_NOTE}
        />
      </div>
    </AiLabShell>
  );
