...
import {
  RECEIVE_PROGRESS_IN_DOTNET_CARDS,
  RECEIVE_PROGRESS_IN_DOTNET_META,
  RECEIVE_PROGRESS_IN_DOTNET_OVERVIEW_NOTE,
  RECEIVE_PROGRESS_IN_DOTNET_OVERVIEW_STEPS,
} from '@/app/lib/taavia-build-version-receive-progress-in-dotnet';
...
export default async function ReceiveProgressInDotnetPage({
  params,
}: {
  ...
  const currentHref = `${overviewHref}/${RECEIVE_PROGRESS_IN_DOTNET_META.slug}`;
    ...
    >
      <div className="ai-lab-page-stack">
        <TaavPageHeader
          ...
          eyebrow="محصول تاویا"
          title={RECEIVE_PROGRESS_IN_DOTNET_META.title}
          description={RECEIVE_PROGRESS_IN_DOTNET_META.description}
          status="active"
          meta={
            <div className="flex flex-wrap items-center gap-2">
              {RECEIVE_PROGRESS_IN_DOTNET_META.pills.map((pill) => (
                <span
                  key={pill}
        ...
        <BuildVersionStepDocClient
          cards={RECEIVE_PROGRESS_IN_DOTNET_CARDS}
          overviewSteps={RECEIVE_PROGRESS_IN_DOTNET_OVERVIEW_STEPS}
          overviewNote={RECEIVE_PROGRESS_IN_DOTNET_OVERVIEW_NOTE}
        />
      </div>
    </AiLabShell>
  );
