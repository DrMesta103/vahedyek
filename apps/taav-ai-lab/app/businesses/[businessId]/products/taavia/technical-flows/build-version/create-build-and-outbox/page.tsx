...
import {
  CREATE_BUILD_AND_OUTBOX_CARDS,
  CREATE_BUILD_AND_OUTBOX_META,
  CREATE_BUILD_AND_OUTBOX_OVERVIEW_NOTE,
  CREATE_BUILD_AND_OUTBOX_OVERVIEW_STEPS,
} from '@/app/lib/taavia-build-version-create-build-and-outbox';
...
export default async function CreateBuildAndOutboxPage({
  params,
}: {
  ...
  const currentHref = `${overviewHref}/${CREATE_BUILD_AND_OUTBOX_META.slug}`;
    ...
    >
      <div className="ai-lab-page-stack">
        <TaavPageHeader
          ...
          eyebrow="محصول تاویا"
          title={CREATE_BUILD_AND_OUTBOX_META.title}
          description={CREATE_BUILD_AND_OUTBOX_META.description}
          status="active"
          meta={
            <div className="flex flex-wrap items-center gap-2">
              {CREATE_BUILD_AND_OUTBOX_META.pills.map((pill) => (
                <span
                  key={pill}
        ...
        <BuildVersionStepDocClient
          cards={CREATE_BUILD_AND_OUTBOX_CARDS}
          overviewSteps={CREATE_BUILD_AND_OUTBOX_OVERVIEW_STEPS}
          overviewNote={CREATE_BUILD_AND_OUTBOX_OVERVIEW_NOTE}
        />
      </div>
    </AiLabShell>
  );
