...
import {
  BUILD_FAILURE_RETRY_RECOVERY_CARDS,
  BUILD_FAILURE_RETRY_RECOVERY_META,
  BUILD_FAILURE_RETRY_RECOVERY_OVERVIEW_NOTE,
  BUILD_FAILURE_RETRY_RECOVERY_OVERVIEW_STEPS,
} from '@/app/lib/taavia-build-version-build-failure-retry-recovery';
...
export default async function BuildFailureRetryRecoveryPage({
  params,
}: {
  ...
  const currentHref = `${overviewHref}/${BUILD_FAILURE_RETRY_RECOVERY_META.slug}`;
    ...
    >
      <div className="ai-lab-page-stack">
        <TaavPageHeader
          ...
          eyebrow="محصول تاویا"
          title={BUILD_FAILURE_RETRY_RECOVERY_META.title}
          description={BUILD_FAILURE_RETRY_RECOVERY_META.description}
          status="active"
          meta={
            <div className="flex flex-wrap items-center gap-2">
              {BUILD_FAILURE_RETRY_RECOVERY_META.pills.map((pill) => (
                <span
                  key={pill}
        ...
        <BuildVersionStepDocClient
          cards={BUILD_FAILURE_RETRY_RECOVERY_CARDS}
          overviewSteps={BUILD_FAILURE_RETRY_RECOVERY_OVERVIEW_STEPS}
          overviewNote={BUILD_FAILURE_RETRY_RECOVERY_OVERVIEW_NOTE}
        />
      </div>
    </AiLabShell>
  );
