...
import {
  GET_TEMPORARY_MEDIA_LINK_CARDS,
  GET_TEMPORARY_MEDIA_LINK_META,
  GET_TEMPORARY_MEDIA_LINK_OVERVIEW_NOTE,
  GET_TEMPORARY_MEDIA_LINK_OVERVIEW_STEPS,
} from '@/app/lib/taavia-build-version-get-temporary-media-link';
...
export default async function GetTemporaryMediaLinkPage({
  params,
}: {
  ...
  const currentHref = `${overviewHref}/${GET_TEMPORARY_MEDIA_LINK_META.slug}`;
    ...
    >
      <div className="ai-lab-page-stack">
        <TaavPageHeader
          ...
          eyebrow="محصول تاویا"
          title={GET_TEMPORARY_MEDIA_LINK_META.title}
          description={GET_TEMPORARY_MEDIA_LINK_META.description}
          status="active"
          meta={
            <div className="flex flex-wrap items-center gap-2">
              {GET_TEMPORARY_MEDIA_LINK_META.pills.map((pill) => (
                <span
                  key={pill}
        ...
        <BuildVersionStepDocClient
          cards={GET_TEMPORARY_MEDIA_LINK_CARDS}
          overviewSteps={GET_TEMPORARY_MEDIA_LINK_OVERVIEW_STEPS}
          overviewNote={GET_TEMPORARY_MEDIA_LINK_OVERVIEW_NOTE}
        />
      </div>
    </AiLabShell>
  );
