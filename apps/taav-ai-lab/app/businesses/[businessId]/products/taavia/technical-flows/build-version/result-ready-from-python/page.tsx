...
import {
  RESULT_READY_FROM_PYTHON_CARDS,
  RESULT_READY_FROM_PYTHON_META,
  RESULT_READY_FROM_PYTHON_OVERVIEW_NOTE,
  RESULT_READY_FROM_PYTHON_OVERVIEW_STEPS,
} from '@/app/lib/taavia-build-version-result-ready-from-python';
...
export default async function ResultReadyFromPythonPage({
  params,
}: {
  ...
  const currentHref = `${overviewHref}/${RESULT_READY_FROM_PYTHON_META.slug}`;
    ...
    >
      <div className="ai-lab-page-stack">
        <TaavPageHeader
          ...
          eyebrow="محصول تاویا"
          title={RESULT_READY_FROM_PYTHON_META.title}
          description={RESULT_READY_FROM_PYTHON_META.description}
          status="active"
          meta={
            <div className="flex flex-wrap items-center gap-2">
              {RESULT_READY_FROM_PYTHON_META.pills.map((pill) => (
                <span
                  key={pill}
        ...
        <BuildVersionStepDocClient
          cards={RESULT_READY_FROM_PYTHON_CARDS}
          overviewSteps={RESULT_READY_FROM_PYTHON_OVERVIEW_STEPS}
          overviewNote={RESULT_READY_FROM_PYTHON_OVERVIEW_NOTE}
        />
      </div>
    </AiLabShell>
  );
