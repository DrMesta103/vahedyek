...
import {
  PUBLISH_PYTHON_OUTBOX_TO_RABBITMQ_CARDS,
  PUBLISH_PYTHON_OUTBOX_TO_RABBITMQ_META,
  PUBLISH_PYTHON_OUTBOX_TO_RABBITMQ_OVERVIEW_NOTE,
  PUBLISH_PYTHON_OUTBOX_TO_RABBITMQ_OVERVIEW_STEPS,
} from '@/app/lib/taavia-build-version-publish-python-outbox-to-rabbitmq';
...
export default async function PublishPythonOutboxToRabbitmqPage({
  params,
}: {
  ...
  const currentHref = `${overviewHref}/${PUBLISH_PYTHON_OUTBOX_TO_RABBITMQ_META.slug}`;
    ...
    >
      <div className="ai-lab-page-stack">
        <TaavPageHeader
          ...
          eyebrow="محصول تاویا"
          title={PUBLISH_PYTHON_OUTBOX_TO_RABBITMQ_META.title}
          description={PUBLISH_PYTHON_OUTBOX_TO_RABBITMQ_META.description}
          status="active"
          meta={
            <div className="flex flex-wrap items-center gap-2">
              {PUBLISH_PYTHON_OUTBOX_TO_RABBITMQ_META.pills.map((pill) => (
                <span
                  key={pill}
        ...
        <BuildVersionStepDocClient
          cards={PUBLISH_PYTHON_OUTBOX_TO_RABBITMQ_CARDS}
          overviewSteps={PUBLISH_PYTHON_OUTBOX_TO_RABBITMQ_OVERVIEW_STEPS}
          overviewNote={PUBLISH_PYTHON_OUTBOX_TO_RABBITMQ_OVERVIEW_NOTE}
        />
      </div>
    </AiLabShell>
  );
