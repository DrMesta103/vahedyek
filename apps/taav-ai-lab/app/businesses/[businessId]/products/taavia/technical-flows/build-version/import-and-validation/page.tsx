...
import {
  IMPORT_AND_VALIDATION_CARDS,
  IMPORT_AND_VALIDATION_META,
  IMPORT_AND_VALIDATION_OVERVIEW_NOTE,
  IMPORT_AND_VALIDATION_OVERVIEW_STEPS,
} from '@/app/lib/taavia-build-version-import-and-validation';
...
export default async function ImportAndValidationPage({
  params,
}: {
  ...
  const currentHref = `${overviewHref}/${IMPORT_AND_VALIDATION_META.slug}`;
    ...
    >
      <div className="ai-lab-page-stack">
        <TaavPageHeader
          ...
          eyebrow="محصول تاویا"
          title={IMPORT_AND_VALIDATION_META.title}
          description={IMPORT_AND_VALIDATION_META.description}
          status="active"
          meta={
            <div className="flex flex-wrap items-center gap-2">
              {IMPORT_AND_VALIDATION_META.pills.map((pill) => (
                <span
                  key={pill}
        ...
        <BuildVersionStepDocClient
          cards={IMPORT_AND_VALIDATION_CARDS}
          overviewSteps={IMPORT_AND_VALIDATION_OVERVIEW_STEPS}
          overviewNote={IMPORT_AND_VALIDATION_OVERVIEW_NOTE}
        />
      </div>
    </AiLabShell>
  );
