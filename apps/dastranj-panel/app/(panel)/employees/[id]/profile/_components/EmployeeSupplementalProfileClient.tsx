...
export function EmployeeSupplementalProfileClient({
  ...
  const workPreferenceValues = categories?.preferences?.values && typeof categories.preferences.values === 'object' ? categories.preferences.values as Record<string, boolean> : {};
  const workPreferenceFields = [
    ['prefersOvertime', 'تمایل به اضافه‌کاری'],
    ['prefersFridayWork', 'تمایل به کار در روز جمعه'],
    ['prefersBusinessTravel', 'تمایل به سفر کاری'],
    ['canRelocate', 'آمادگی جابه‌جایی محل کار'],
    ['prefersRemote', 'تمایل به دورکاری'],
    ['prefersOnSite', 'تمایل به کار حضوری'],
    ['prefersHybrid', 'تمایل به کار ترکیبی'],
    ['prefersTeamWork', 'تمایل به کار تیمی'],
    ['prefersIndividualWork', 'تمایل به کار فردی'],
    ['prefersManagementRole', 'تمایل به نقش مدیریتی'],
    ['prefersExecutionRole', 'تمایل به نقش اجرایی'],
  ] as const;
  ...
  return (
    <div className="employee-supplemental-profile-page" dir="rtl" lang="fa">
      ...
      {categories?.canUpdate ? <section className="employee-detail-section employee-work-preferences"><div className="employee-detail-section-head"><h2>ترجیحات کاری</h2><p>این موارد ترجیح کارمند هستند و تعهد قطعی محسوب نمی‌شوند.</p></div><form action={saveEmployeeWorkPreferencesAction} className="employee-work-preferences-form"><input type="hidden" name="employeeId" value={employee.id}/><div className="employee-work-preferences-grid">{workPreferenceFields.map(([key, label]) => <label key={key} className="employee-work-preference-option"><input type="checkbox" name={key} value="true" defaultChecked={workPreferenceValues[key] === true}/><span>{label}</span></label>)}</div><button className="employee-work-preferences-submit">ذخیره ترجیحات</button></form></section> : null}
