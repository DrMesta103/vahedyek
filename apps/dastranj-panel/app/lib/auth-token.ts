...
export const AUTH_COOKIE = 'dastranj_auth';
...
const secret = process.env.AUTH_JWT_SECRET ?? 'dev-insecure-jwt-secret-dastranj';
...
export async function createAuthToken(payload: AuthTokenPayload) {
  const ttl = payload.state === 'active' ? ACTIVE_TTL : PENDING_TTL;
...
export async function verifyAuthToken(token: string): Promise<AuthTokenPayload | null> {
  try {
    ...
    return { userId: payload.userId, tenantId: payload.tenantId ?? null, state: payload.state };
  } catch {
    return null;
  }
}
...
export function setAuthCookie(response: NextResponse, session: { token: string; expiresAt: Date }) {
  response.cookies.set(AUTH_COOKIE, session.token, {
    httpOnly: true, sameSite: 'lax', secure: false, path: '/', expires: session.expiresAt,
  });
}
