...
function readDatabaseUrlFromEnvFile() {
  const candidates = [
    ...
    path.resolve(process.cwd(), 'apps/dastranj-panel/.env'),
  ];
...
export function getCurrentDatabaseUrl() {
  ...
  throw new Error('Missing DATABASE_URL for dastranj-panel.');
}
...
export const databaseIsolationNote = 'dastranj-panel reads DATABASE_URL from apps/dastranj-panel/.env first.';
