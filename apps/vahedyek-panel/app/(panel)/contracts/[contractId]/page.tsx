...
import { buildReceiptAllocation } from '../../../lib/contractReceiptAllocation';
import { getReceiptsStorageKey, normalizeReceiptRecords, type RegisteredReceiptRecord } from '../../../lib/contractReceipts';
import { toComparableDateFromDueString } from '../../../lib/financialUtils';
...
function getFinancialToneClass(tone: 'emerald' | 'amber' | 'rose' | 'slate') {
  switch (tone) {
    case 'emerald':
...
function getInstallmentOriginLabel(categoryId: string | null | undefined, categoryTitle: string | null | undefined) {
...
export default function ContractDetailsPage() {
  ...
  const financialSummary = useMemo(() => {
    ...
    if (!financial) {
      return {
        contractTotalRial: null,
        confirmedPaidRial: null,
        pendingReviewRial: null,
        principalRemainingRial: null,
        ...
        gaps: ['داده‌ی مالی این قرارداد هنوز موجود نیست.'],
      };
    }

    const penaltyTimeline = buildContractPenaltyTimeline({
      financial,
      penalties,
      receipts: registeredReceipts,
    });
    const receiptAllocation = buildReceiptAllocation({
      buckets: penaltyTimeline.combinedBuckets,
      receipts: registeredReceipts,
    });

    const today = new Date();
    today.setHours(0, 0, 0, 0);
    const dueSummaries = Object.values(receiptAllocation.dueById);
    ...
    const penaltyPaidRial = penaltySummaries.reduce((sum, summary) => sum + summary.paidAmountRial, 0);
    const openPenaltyRial = Math.max(0, penaltyTotalRial - penaltyPaidRial);
    const hasUnknownDueDates = principalSummaries.some((summary) => !toComparableDateFromDueString(summary.row.dueDate));

    const settlementStatus = (() => {
      if (principalRemainingRial <= 0 && openPenaltyRial <= 0) {
        return { label: 'تسویه کامل', tone: 'emerald' as const };
      }
      if (overdueDebtRial > 0 && openPenaltyRial > 0) {
        return { label: 'دارای معوقه', tone: 'rose' as const };
      }
      if (overdueDebtRial > 0) {
        return { label: 'دارای بدهی سررسیدشده', tone: 'rose' as const };
      }
      if (futureDebtRial > 0) {
    ...
    const gaps = [
      'در مدل فعلی، وضعیت تأیید یا بررسی جداگانه برای هر رسید ذخیره نمی‌شود و مبلغ پرداخت‌شده بر پایه رسیدهای ثبت‌شده یا تخصیص‌یافته محاسبه می‌شود.',
      'سیاست محاسبه‌ی آمادگی مالی برای تحویل یا سند در داده‌ی فعلی موجود نیست.',
    ];

    if (hasUnknownDueDates) {
      gaps.push('تاریخ برخی سررسیدها به‌صورت قطعی قابل تشخیص نبود.');
    }

    return {
      contractTotalRial: penaltyTimeline.contractBaseTotalRial,
      confirmedPaidRial: receiptAllocation.totalAllocatedRial,
      pendingReviewRial: null,
  ...
  const installmentTableRows = useMemo(() => {
    const financial = contract?.data?.financial ?? null;
    const penalties = contract?.data?.penalties ?? null;
    ...
    if (!financial) {
      return {
        ...
        gaps: ['داده‌ی مالی قرارداد برای ساخت جدول اقساط موجود نیست.'],
      };
    }
    ...
    const penaltyByPrincipalDueId = new Map<string, number>();
    for (const row of penaltyTimeline.penaltyRows) {
      const key = String(row.principalDueRowId ?? '').trim();
      if (!key) continue;
      penaltyByPrincipalDueId.set(key, (penaltyByPrincipalDueId.get(key) ?? 0) + Number(row.amount ?? 0));
    }

    const rows = penaltyTimeline.principalRows.map((row, index) => {
      const summary = receiptAllocation.dueById[row.id];
      const paidRial = summary?.paidAmountRial ?? 0;
      const remainingRial = summary?.remainingAmountRial ?? Math.max(0, Number(row.amount ?? 0) - paidRial);
      const dueDate = toComparableDateFromDueString(row.dueDate);
      const isFuture = Boolean(dueDate && dueDate > today);
      const isPast = Boolean(dueDate && dueDate < today);
      const isToday = Boolean(dueDate && dueDate.getTime() === today.getTime());
      const penaltyRial = penaltyByPrincipalDueId.get(row.id) ?? 0;
      const delayDays = isPast ? Math.max(1, Math.floor((today.getTime() - dueDate!.getTime()) / 86_400_000)) : null;

      let statusLabel: string;
      let statusTone: 'emerald' | 'amber' | 'rose' | 'slate';
      if (!dueDate) {
        statusLabel = 'نامشخص';
        statusTone = 'slate';
      } else if (remainingRial <= 0) {
        statusLabel = 'پرداخت‌شده';
        statusTone = 'emerald';
      } else if (paidRial > 0) {
        statusLabel = 'پرداخت ناقص';
        statusTone = 'amber';
      } else if (isFuture) {
        statusLabel = 'آینده';
        statusTone = 'slate';
      } else if (isToday || (!isPast && !isFuture)) {
        statusLabel = 'سررسیدشده';
        statusTone = 'amber';
      } else {
        statusLabel = 'معوق';
        statusTone = penaltyRial > 0 ? 'rose' : 'amber';
      }

      if (penaltyRial > 0 && remainingRial > 0 && statusLabel !== 'پرداخت‌شده') {
        statusLabel = statusLabel === 'معوق' ? 'مشمول جریمه' : `${statusLabel} · مشمول جریمه`;
        statusTone = 'rose';
      }
      ...
      return {
        ...
        index: index + 1,
        title: row.title,
        originLabel: getInstallmentOriginLabel(row.categoryId, row.categoryTitle),
    ...
    const gaps = ['رسیدهای در انتظار بررسی در مدل فعلی به‌صورت جداگانه برچسب‌گذاری نمی‌شوند؛ برای تفکیک دقیق این وضعیت، اطلاعات بررسی رسید باید کامل‌تر ثبت شود.'];
  ...
  return (
    ...
    <main className="contract-details-page" dir="rtl" lang="fa">
      ...
      <section className="contract-details-panel mt-4">
        <div dir="rtl" className="rounded-[22px] border border-slate-200/80 bg-white/90 px-4 py-4 shadow-sm">
          <div className="flex flex-col gap-2 border-b border-slate-100 pb-3">
            <div className="text-[17px] font-black text-slate-900">خلاصه مالی قرارداد</div>
            <p className="text-[12px] leading-6 text-slate-500">
              این نما فقط برای همین قرارداد محاسبه می‌شود و از داده‌های مالی قرارداد و رسیدهای ثبت‌شده‌ی همین قرارداد استفاده
              می‌کند.
            </p>
          </div>
            ...
            {[
              {
                ...
                value: formatMoneyRialOrUnknown(financialSummary.contractTotalRial),
                tone: 'slate' as const,
              },
              {
                label: 'پرداخت‌شده تأییدشده',
                value: formatMoneyRialOrUnknown(financialSummary.confirmedPaidRial),
                tone: 'emerald' as const,
              },
              {
                label: 'مبلغ در انتظار بررسی',
                value:
                  financialSummary.pendingReviewRial == null
                    ? 'نامشخص'
                    : formatMoneyRialOrUnknown(financialSummary.pendingReviewRial),
                tone: 'amber' as const,
              },
              {
                label: 'مانده بدهی',
                value: formatMoneyRialOrUnknown(financialSummary.principalRemainingRial),
                tone: 'rose' as const,
              },
              {
                label: 'مبلغ معوق',
                value: formatMoneyRialOrUnknown(financialSummary.overdueDebtRial),
                tone: 'rose' as const,
              },
              {
                label: 'جریمه باز',
                value: formatMoneyRialOrUnknown(financialSummary.openPenaltyRial),
                tone: 'amber' as const,
              },
              {
                label: 'وضعیت تسویه',
                value: financialSummary.settlementStatus.label,
                tone: financialSummary.settlementStatus.tone,
              },
              {
                label: 'آمادگی مالی برای تحویل یا سند',
                value: financialSummary.readinessStatus.label,
                tone: financialSummary.readinessStatus.tone,
              },
            ].map((item) => (
              <div key={item.label} className={`rounded-2xl border px-4 py-3 ${getFinancialToneClass(item.tone)}`}>
                <div className="text-[12px] font-bold opacity-85">{item.label}</div>
                <div className="mt-2 text-[15px] font-black leading-7">{item.value}</div>
              </div>
            ))}
          </div>

          {financialSummary.gaps.length ? (
            <div className="mt-4 rounded-2xl border border-dashed border-amber-200 bg-amber-50/70 px-4 py-3">
              <div className="text-[12px] font-black text-amber-900">موارد نیازمند تکمیل در وضعیت مالی</div>
              <ul className="mt-2 space-y-1 text-[12px] leading-6 text-amber-900/90">
                {financialSummary.gaps.map((gap) => (
                  <li key={gap}>• {gap}</li>
                ))}
              </ul>
            </div>
      ...
      <section className="contract-details-panel mt-4">
        <div dir="rtl" className="rounded-[22px] border border-slate-200/80 bg-white/90 px-4 py-4 shadow-sm">
          <div className="flex flex-col gap-2 border-b border-slate-100 pb-3">
            <div className="text-[17px] font-black text-slate-900">جدول اقساط قرارداد</div>
            <p className="text-[12px] leading-6 text-slate-500">
              این جدول فقط اقساط و تعهدات پرداختی همین قرارداد را نشان می‌دهد. مبلغ پرداخت‌شده از تخصیص رسیدهای ثبت‌شده محاسبه
              می‌شود و مانده، تأخیر و جریمه از همان داده‌های مالی قرارداد استخراج می‌شوند.
            </p>
          </div>

          {installmentTableRows.rows.length ? (
            <div className="mt-4 overflow-x-auto rounded-2xl border border-slate-200/90">
              <table className="w-full min-w-[1100px] border-collapse text-[11px]" dir="rtl">
                <thead>
                  <tr className="border-b border-slate-200 bg-slate-50 text-slate-600">
                    <th className="px-3 py-3 text-right font-black">#</th>
                    <th className="min-w-[16rem] px-3 py-3 text-right font-black">عنوان / منشأ</th>
                    <th className="px-3 py-3 text-center font-black">مبلغ قسط</th>
                    <th className="px-3 py-3 text-center font-black">تاریخ سررسید</th>
                    <th className="px-3 py-3 text-center font-black">پرداخت‌شده</th>
                    <th className="px-3 py-3 text-center font-black">مانده</th>
                    <th className="px-3 py-3 text-center font-black">وضعیت قسط</th>
                    <th className="px-3 py-3 text-center font-black">تأخیر</th>
                    <th className="px-3 py-3 text-center font-black">جریمه مرتبط</th>
                  </tr>
                </thead>
                <tbody>
                  {installmentTableRows.rows.map((row, idx) => (
                    <tr key={row.id} className={`border-b border-slate-100 ${idx % 2 === 1 ? 'bg-slate-50/40' : ''}`}>
                      <td className="px-3 py-3 text-right font-black text-slate-900">{row.index.toLocaleString('fa-IR')}</td>
                      <td className="px-3 py-3 text-right">
                        <div className="font-bold text-slate-900">{row.title}</div>
                        <div className="mt-1 text-[10px] font-semibold text-slate-500">منشأ: {row.originLabel}</div>
                      </td>
                      <td className="px-3 py-3 text-center tabular-nums font-black text-slate-900">
                        {formatMoneyRialOrUnknown(row.amountRial)}
                      </td>
                      <td className="px-3 py-3 text-center tabular-nums font-semibold text-slate-700">{row.dueDate}</td>
                      <td className="px-3 py-3 text-center tabular-nums font-black text-emerald-700">
                        {formatMoneyRialOrUnknown(row.paidRial)}
                      </td>
                      <td className="px-3 py-3 text-center tabular-nums font-black text-rose-700">
                        {formatMoneyRialOrUnknown(row.remainingRial)}
                      </td>
                      <td className="px-3 py-3 text-center">
                        <span className={`inline-flex rounded-full border px-2.5 py-1 text-[10px] font-black ${getFinancialToneClass(row.statusTone)}`}>
                          {row.statusLabel}
                        </span>
                      </td>
                      <td className="px-3 py-3 text-center tabular-nums font-black text-slate-800">
                        {row.delayDays == null ? '—' : row.delayDays.toLocaleString('fa-IR')}
                      </td>
                      <td className="px-3 py-3 text-center tabular-nums font-black text-rose-700">
                        {row.penaltyRial > 0 ? formatMoneyRialOrUnknown(row.penaltyRial) : '—'}
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            <div className="mt-4 rounded-2xl border border-dashed border-slate-200 bg-slate-50/40 px-4 py-10 text-center text-[13px] font-semibold text-slate-500">
              برای این قرارداد هیچ قسط یا تعهد پرداختی در داده مالی ثبت نشده است.
            </div>
          )}

          {installmentTableRows.gaps.length ? (
            <div className="mt-4 rounded-2xl border border-dashed border-amber-200 bg-amber-50/70 px-4 py-3">
              <div className="text-[12px] font-black text-amber-900">موارد نیازمند تکمیل در جدول اقساط</div>
              <ul className="mt-2 space-y-1 text-[12px] leading-6 text-amber-900/90">
                {installmentTableRows.gaps.map((gap) => (
                  <li key={gap}>• {gap}</li>
                ))}
              </ul>
            </div>
