...
function normalizeGroups(groups: ContractTechnicalSpecGroup[] | null | undefined) {
...
function getGroupSpecs(group: ContractTechnicalSpecGroup, projectSpecs: TechnicalSpecItem[]) {
...
export function TechnicalSpecsStep({ title }: { title: string }) {
  ...
  const [groups, setGroups] = useState<ContractTechnicalSpecGroup[]>([]);
  ...
  const payload: ContractTechnicalSpecsPayload = useMemo(
    () => ({
      specs: groups.map((group) => ({
        id: group.id,
        title: group.title.trim(),
        selectedSpecIds: group.selectedSpecIds.filter(Boolean),
      })),
    }),
    [groups],
  );
  ...
  function openEditDialog(group: ContractTechnicalSpecGroup) {
    setForm({
      title: group.title,
  ...
  async function submitDialog() {
    ...
    try {
      ...
      if (!response.ok) throw new Error('ذخیره مشخصات فنی ناموفق بود.');
      ...
      setFormError(error instanceof Error ? error.message : 'ذخیره مشخصات فنی ناموفق بود.');
    } finally {
      setSaving(false);
    }
  }

  const canSubmit = form.title.trim().length > 0 && form.selectedSpecIds.length > 0;
      ...
      <section className="rounded-[28px] border border-slate-200 bg-white px-5 py-5 shadow-[0_14px_30px_rgba(15,23,42,0.05)]">
        <div className="flex flex-wrap items-start justify-between gap-4 border-b border-slate-100 pb-4">
          <div className="max-w-[760px] text-right">
            ...
            <p className="mt-2 text-[13px] leading-7 text-slate-500">
              در این بخش برای هر گروه، مشخصه فنی بسازید و بعد از بین مشخصه‌های فنی تعریف‌شده در مجتمع، موارد مرتبط را انتخاب کنید.
            </p>
          </div>
        ...
        {!loading && !visibleGroups.length ? (
          <div className="business-blocks-state">
            هنوز گروهی ثبت نشده است. برای شروع، یک گروه مشخصه فنی بسازید و مشخصه‌های مرتبط را انتخاب کنید.
          </div>
        ) : null}
          ...
          >
            <div className="px-8 pt-8 text-right">
              <h2 id="technical-specs-dialog-title" className="text-[26px] font-black leading-10 text-slate-700">
                {dialog.mode === 'edit' ? 'ویرایش مشخصه فنی قرارداد' : 'افزودن مشخصه فنی قرارداد'}
              </h2>
              <p className="mt-4 text-[13px] leading-7 text-slate-600">
                عنوان گروه را وارد کنید و سپس از بین مشخصه‌های فنی تعریف‌شده در مجتمع، موارد مرتبط را انتخاب کنید.
              </p>
            </div>
              ...
              <div className="grid gap-3 text-right">
                <div>
                  ...
                  <p className="mt-1 text-[12px] leading-6 text-slate-500">
                    برای هر گروه می‌توانید چند مشخصه فنی انتخاب کنید. موارد انتخابی به همان شکل داخل قرارداد ثبت می‌شوند.
                  </p>
                </div>
