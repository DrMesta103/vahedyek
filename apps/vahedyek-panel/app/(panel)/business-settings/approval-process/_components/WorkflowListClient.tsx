...
function uniqueLabels(ids: string[], users: UserOpt[]) {
  const labels = ids.map((id) => users.find((user) => user.id === id)?.label ?? id);
...
export function WorkflowListClient() {
  ...
  const usedUsageKeys = useMemo(() => {
    ...
    for (const workflow of items) {
      for (const key of workflow.usageTypes ?? []) set.add(String(key));
    }
    return set;
  }, [items]);

  const firstUnusedUsageKey = useMemo(() => {
    return (approvalUsageOptions.map((item) => item.id).find((key) => !usedUsageKeys.has(key)) ?? null) as ApprovalUsageKey | null;
  }, [usedUsageKeys]);
  ...
  const cards = useMemo(
    () =>
      approvalUsageOptions.flatMap((usage) => {
        ...
        const workflow = items.find((item) => (item.usageTypes ?? []).includes(usageKey));
        if (!workflow) return [];
        ...
        const finalProcessName = workflow.finalApproverUserId
          ? users.find((user) => user.id === workflow.finalApproverUserId)?.label ?? workflow.finalApproverUserId
          : '';
        ...
        return [
          {
            ...
            processingHint: steps[0]?.type === 'SEQUENTIAL' ? 'ترتیب مراحل رعایت می‌شود' : 'مراحل مستقل تایید می‌گیرند',
            ...
            buyerText: workflow.buyerShouldApprove ? 'خریدار هم تایید می‌کند' : 'بدون تایید خریدار',
            finalText: finalProcessName ? `نهایی کل: ${finalProcessName}` : 'بدون نهایی کل',
            updatedText: new Intl.DateTimeFormat('fa-IR', { month: 'short', day: 'numeric' }).format(new Date(workflow.updatedAt)),
          },
        ];
      }),
    [items, users],
    ...
    <div className="mx-auto w-full max-w-6xl px-4 pb-24 pt-4 sm:px-6 lg:px-8" dir="rtl" lang="fa">
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="text-right">
          <h1 className="text-2xl font-black text-[var(--text-strong)]">فرایندهای تایید</h1>
          <p className="mt-2 max-w-2xl text-sm leading-7 text-[var(--text-muted)]">
            برای هر نوع کاربری واحد، یک مسیر تایید تعریف کنید. ساخت فرایند تا زمان ذخیره نهایی در صفحه مدیریت انجام نمی‌شود.
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="space-y-4">
        {cards.map((workflow) => (
          <SectionCard key={workflow.id} className="group overflow-visible border-slate-200 bg-white shadow-sm transition hover:border-teal-200 hover:shadow-md">
            <div className="p-3">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0 flex-1 text-right">
                  <h2 className="text-[15px] font-black leading-6 text-[var(--text-strong)]">{workflow.title}</h2>
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="inline-flex rounded-full border border-teal-100 bg-teal-50 px-3 py-1 text-[11px] font-bold text-teal-700">{workflow.usageText}</span>
                    <span className="rounded-full bg-slate-50 px-3 py-1 text-[11px] font-normal text-slate-500">{workflow.processingText}</span>
                    <span className="rounded-full bg-slate-50 px-3 py-1 text-[11px] font-normal text-slate-500">{workflow.buyerText}</span>
                    <span className="rounded-full bg-slate-50 px-2.5 py-1 text-[10px] font-normal text-slate-500">
                      <b className="text-[var(--text-strong)]">{workflow.stepCount}</b> مرحله
                    </span>
                    <span className="rounded-full bg-slate-50 px-2.5 py-1 text-[10px] font-normal text-slate-500">
                      <b className="text-[var(--text-strong)]">{workflow.approverCount}</b> تاییدکننده
                    </span>
                    <span className="rounded-full bg-slate-50 px-2.5 py-1 text-[10px] font-normal text-slate-500">
                      <b className="text-[var(--text-strong)]">{workflow.finalStepApproverCount}</b> نهایی
                    </span>
                  </div>
                </div>
                  ...
                  <div className="rounded-[8px] border border-slate-100 px-3 py-1.5">
                    ...
                    <div className="mt-1.5 flex flex-wrap gap-1.5">
                      {workflow.approverPreview.length ? (
                        workflow.approverPreview.map((name) => (
                      ...
                      {workflow.hasMoreApprovers ? <span className="rounded-full bg-slate-100 px-2 py-1 text-[10px] text-slate-500">+{workflow.approverNames.length - 4}</span> : null}
                    </div>
                  </div>
                </div>
                ...
                <div className="grid gap-2 text-right text-[11px] font-normal text-slate-600">
                  <div className="rounded-[8px] bg-slate-50/80 px-3 py-1.5">
                    ...
                    <div className="mt-1.5 flex items-center gap-2">
                      ...
                      <span className="truncate">{workflow.finalText}</span>
                    </div>
                    {workflow.finalStepNames.length ? (
                      <div className="mt-1.5 truncate text-[10px] leading-5 text-slate-500">
                        نهایی مرحله: {workflow.finalStepNames.slice(0, 2).join('، ')}
                        {workflow.finalStepNames.length > 2 ? ' و دیگران' : ''}
                      </div>
                    ) : null}
                    <div className="mt-1.5 truncate text-[10px] leading-5 text-slate-500">
                      {workflow.allMustApproveCount} تایید کامل، {workflow.minimumLogicCount} حد نصاب
