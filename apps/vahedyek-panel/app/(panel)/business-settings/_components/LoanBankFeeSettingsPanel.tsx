...
const FEE_MODE_OPTIONS: Array<{
...
}> = [
  {
    value: 'fixed',
    title: 'مبلغ ثابت',
    description: 'کارمزد بانک به‌صورت مبلغ ثابت ثبت می‌شود.',
    icon: CircleDollarSign,
  },
  {
    value: 'percent',
    title: 'درصدی از مبلغ وام',
    description: 'کارمزد بانک به‌صورت درصدی از مبلغ وام محاسبه می‌شود.',
    icon: Percent,
  },
  {
    value: 'combined',
    title: 'ترکیب مبلغ ثابت و درصد',
    description: 'کارمزد بانک می‌تواند هم‌زمان شامل مبلغ ثابت و درصد باشد.',
    icon: Combine,
  },
];
...
export function LoanBankFeeSettingsPanel() {
    ...
    const load = async () => {
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(payload.message || 'دریافت تنظیمات کارمزد بانک ناموفق بود.');
        }
        const payload = (await response.json()) as LoanSettingsState;
        if (mounted) setState(payload);
      } catch (loadError) {
        if (mounted) setError(loadError instanceof Error ? loadError.message : 'دریافت تنظیمات کارمزد بانک ناموفق بود.');
      } finally {
  ...
  const handleSave = async () => {
    ...
    if (!state.loanBankFeeBankPolicyEnabled && !state.loanBankFeeValue.trim()) {
      setError('مقدار کارمزد بانک را وارد کنید.');
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        const payload = (await response.json().catch(() => ({}))) as { message?: string };
        throw new Error(payload.message || 'ذخیره تنظیمات کارمزد بانک ناموفق بود.');
      }

      setMessage('تنظیمات کارمزد بانک با موفقیت ذخیره شد.');
    } catch (saveError) {
      setError(saveError instanceof Error ? saveError.message : 'ذخیره تنظیمات کارمزد بانک ناموفق بود.');
    } finally {
      setSaving(false);
    }
  };
  ...
  if (loading || !state) {
    return <LoanLoadingState label="در حال دریافت تنظیمات کارمزد بانک..." />;
  }
  ...
  return (
    <section className="mx-auto w-full max-w-4xl px-4 pb-28 pt-4 sm:px-6 lg:px-8">
      <div className="border-b border-[color:var(--border-soft)] pb-4 text-center">
        <h1 className="text-xl font-black text-[color:var(--text-strong)] sm:text-2xl">کارمزد بانک</h1>
        <p className="mt-3 text-sm leading-8 text-[color:var(--text-muted)]">
          در این بخش می‌توانید نحوه محاسبه و پرداخت کارمزد بانک را تنظیم کنید.
        </p>
      </div>
      ...
      <section className="mt-4 rounded-[8px] bg-[color:var(--surface)] px-4 py-6 text-right">
        <p className="text-sm leading-8 text-[color:var(--text-muted)]">
          در این قسمت مشخص می‌کنید چه کسی کارمزد بانک را پرداخت می‌کند و آیا بانک در محاسبه دخالت دارد یا نه.
        </p>
        <h2 className="mt-2 text-[20px] font-black text-[color:var(--text-strong)]">مسئولیت پرداخت کارمزد بانک</h2>
      </section>

      <section className="mt-4 space-y-6 rounded-[8px] bg-[color:var(--surface)] px-4 py-6">
        <ResponsibilityRow
          title="سهم خریدار"
          description="میزان سهم خریدار در پرداخت کارمزد بانک را مشخص کنید."
        ...
        <ResponsibilityRow
          title="سهم فروشنده"
          description="میزان سهم فروشنده در پرداخت کارمزد بانک را مشخص کنید."
        ...
        <div dir="ltr" className="flex items-start justify-between gap-4">
          ...
          <div dir="rtl" className="space-y-3 text-right">
            <h2 className="text-[20px] font-black leading-8 text-[color:var(--text-strong)]">
              سیاست بانک درباره کارمزد را فعال یا غیرفعال کنید.
            </h2>
            <p className="text-sm leading-7 text-[color:var(--text-muted)]">
              اگر سیاست بانک فعال باشد، مقدار کارمزد بر اساس تنظیمات سامانه محاسبه می‌شود.
            </p>
          </div>
        </div>
      </section>
        ...
        <>
          ...
          <section className="mt-4 rounded-[8px] bg-[color:var(--surface)] px-4 py-6">
            <div className="space-y-4 text-right">
              <FieldLabel label="مبلغ کارمزد بانک" required />
              <FinancialAmountInput
              ...
              <p className="text-sm text-[color:var(--text-muted)]">
                مقدار کارمزد را بر اساس حالت انتخاب‌شده وارد کنید. اگر حالت درصدی است، فقط عدد درصد را بنویسید.
              </p>
            </div>
          </section>
        </>
