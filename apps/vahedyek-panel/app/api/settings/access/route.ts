...
function legacyMenuIdsToPermissionKeys(menuItemIds: string[]) {
  const permissionKeys = new Set<string>();
  const byId = new Map(MENU_PERMISSION_ITEMS.map((item) => [item.id, item]));

  for (const menuItemId of menuItemIds) {
    const permissionKey = byId.get(menuItemId)?.requiredPermission;
    if (permissionKey) permissionKeys.add(permissionKey);
  }
...
async function getAccessPayload(tenantId: string) {
  ...
  return {
    permissions: APP_PERMISSION_ITEMS,
    menuItems: MENU_PERMISSION_ITEMS,
    roles: roles.map((role) => {
      const permissionKeys = role.permissions.map((permission) => permission.permissionKey);
      const allowedMenuItemIds = filterMenuByPermissions(MENU_PERMISSION_ITEMS, { isOwner: false, permissionKeys }).map((item) => item.id);
    ...
    representatives: representatives.map((representative) => {
      const panelAccessEligible = representative.hasSigningAuthority && representative.panelAccessEnabled;
