...
export function AppendixTimelineCard({
  ...
  return (
    <article className="relative overflow-visible rounded-[8px] border border-slate-200/80 bg-white shadow-[0_16px_40px_-30px_rgba(15,23,42,0.24)] transition hover:-translate-y-0.5 hover:border-[color-mix(in_srgb,var(--dark-teal)_24%,transparent)] hover:shadow-[0_24px_48px_-30px_rgba(15,23,42,0.3)]">
      <div ref={menuRef} className="absolute right-4 top-4 z-20">
        <button
          ...
          className="inline-flex h-10 w-10 items-center justify-center rounded-[8px] border border-slate-200 bg-white text-slate-500 shadow-sm transition hover:bg-slate-50"
        >
          <MoreHorizontal className="h-4 w-4" />
        </button>

        {menuOpen ? (
          <div className="absolute right-0 mt-2 w-44 overflow-hidden rounded-[8px] border border-slate-200 bg-white p-1 shadow-[0_18px_45px_-20px_rgba(15,23,42,0.24)]">
            <button
              type="button"
              onClick={(event) => {
                event.stopPropagation();
              ...
              className="flex h-11 w-full items-center justify-between rounded-[8px] px-3 text-right text-[12px] font-extrabold text-slate-700 transition hover:bg-slate-50"
            >
            ...
            {appendix.canDelete ? (
              <button
                ...
                className="flex h-11 w-full items-center justify-between rounded-[8px] px-3 text-right text-[12px] font-extrabold text-rose-700 transition hover:bg-rose-50"
              >
                <span>حذف</span>
      ...
      <button type="button" onClick={onView} className="flex w-full flex-col gap-4 px-5 py-5 pr-16 text-right sm:px-6 sm:pr-20">
        ...
        <div className="grid gap-3 md:grid-cols-2">
          <div className="flex items-center justify-between gap-3 rounded-[8px] bg-slate-50/80 px-4 py-3">
          ...
          <div className="md:col-span-2 flex items-center justify-between gap-3 rounded-[8px] bg-[color-mix(in_srgb,var(--dark-teal)_05%,white)] px-4 py-3">
          ...
          {appendix.notes ? (
            <div className="md:col-span-2 rounded-[8px] border border-slate-100 bg-white px-4 py-3">
