...
function StatRow({ label, value, accent = false }: { label: string; value: string; accent?: boolean }) {
  return (
    <div className="flex items-center justify-between gap-3 rounded-2xl border border-slate-100/90 bg-white/95 px-4 py-3">
          ...
          return (
            <div key={version.id} className="flex items-center gap-3">
              <button
                ...
                className={`group flex w-[min(280px,78vw)] flex-col rounded-[22px] border px-4 py-3 text-right transition ${
                  selected
                ...
                <div className="mt-2 flex flex-wrap items-center justify-end gap-2">
                  ...
                  {version.compareHref ? (
                    <Link
                      ...
                      onClick={(e) => e.stopPropagation()}
                      className="inline-flex items-center gap-1 rounded-lg border border-cyan-200 bg-cyan-50 px-2 py-1 text-[10px] font-bold text-cyan-800 hover:bg-cyan-100"
                    >
                      تاریخچه بخش‌ها
                      <ExternalLink className="h-3 w-3" />
                    </Link>
...
export function HistoryTimelineView({
  ...
  return (
    <section
      className={`contract-history-panel overflow-hidden rounded-[28px] border border-slate-200/80 bg-white shadow-[0_18px_42px_-34px_rgba(15,23,42,0.2)] ${
        embedded ? '' : 'p-5 sm:p-6'
      }`}
      dir="rtl"
      lang="fa"
    >
      <div className="rounded-[24px] border border-slate-100 bg-[linear-gradient(180deg,rgba(248,250,252,0.98),rgba(241,245,249,0.84))] px-4 py-4 sm:px-5 sm:py-5">
      ...
      <div className="mt-4 flex flex-col gap-3 rounded-[20px] border border-slate-200/80 bg-white/90 p-3 sm:flex-row sm:items-center sm:justify-between">
        <label className="relative block min-w-0 flex-1">
          ...
          <input
            ...
            className="h-11 w-full rounded-2xl border border-slate-200 bg-slate-50/80 pr-10 pl-3 text-right text-[13px] font-semibold text-slate-700 outline-none transition focus:border-[color-mix(in_srgb,var(--dark-teal)_35%,transparent)] focus:bg-white"
          />
        </label>
        <div className="flex flex-wrap items-center justify-end gap-2">
          <button
            ...
            className={`inline-flex h-11 items-center gap-2 rounded-2xl border px-3 text-[12px] font-bold transition ${
          ...
          <button
            ...
            className="inline-flex h-11 items-center gap-2 rounded-2xl border border-slate-200 bg-white px-3 text-[12px] font-bold text-slate-700 hover:bg-slate-50"
          >
        ...
        {grouped.length ? (
          grouped.map((group) => (
            <div key={group.groupKey} className="space-y-3">
              <div className="sticky top-0 z-[1] rounded-2xl border border-slate-200/70 bg-white/95 px-4 py-2.5 text-right backdrop-blur-sm">
                <div className="text-[13px] font-black text-slate-800">{group.title}</div>
                <div className="text-[11px] font-semibold text-slate-500">{group.items.length.toLocaleString('fa-IR')} بخش</div>
              </div>
          ...
          <div className="rounded-[22px] border border-dashed border-slate-200 bg-slate-50/80 px-6 py-10 text-center">
      ...
      <div className="mt-5 rounded-[22px] border border-slate-200 bg-slate-50/70 px-4 py-3 text-right">
