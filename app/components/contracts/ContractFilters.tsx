...
const selectStyle: React.CSSProperties = {
  ...
  padding: '8px 12px', fontFamily: 'inherit', fontSize: '13px',
  color: '#4b5563', background: '#fff', outline: 'none', width: '100%',
};

const labelStyle: React.CSSProperties = {
  fontSize: '11px', color: '#9ca3af', marginBottom: '4px', display: 'block',
};
  ...
  return (
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '12px', alignItems: 'flex-end' }}>
      ...
      <div style={{ minWidth: '130px' }}>
        ...
        <input type="text" value={filters.dateFrom ?? ''} onChange={(e) => handleChange('dateFrom', e.target.value)}
          placeholder="۱۴۰۰/۰۱/۰۱" style={selectStyle}
      ...
      <div style={{ minWidth: '130px' }}>
        ...
        <input type="text" value={filters.dateTo ?? ''} onChange={(e) => handleChange('dateTo', e.target.value)}
          placeholder="۱۴۰۳/۱۲/۲۹" style={selectStyle}
          onFocus={(e) => e.target.style.borderColor = 'var(--dark-teal)'}
          onBlur={(e) => e.target.style.borderColor = '#d1d5db'} />
      </div>
      ...
      <div style={{ minWidth: '120px' }}>
        ...
        <select value={filters.unitId ?? ''} onChange={(e) => handleChange('unitId', e.target.value)}
      ...
      {hasActiveFilters && (
        <button onClick={onClearFilters} style={{
          ...
          color: '#ef4444', borderRadius: '20px', cursor: 'pointer',
        }}>
          پاک کردن فیلترها
        </button>
      )}
